...
- **Credamo 见数作答日期时间窗**: 对齐原项目 `main@9137256`，支持 `answer_datetime_window` 校验与提交时间采样
...
- 文本随机模式、地区文本、多空文本
- AI 主观题作答
- 作答时长与提交间隔配置；默认作答时长对齐原项目为 60-120 秒
...
Go 版本保持纯 HTTP provider 路线，不迁移或保留 browser/Playwright/Selenium 兼容层；原项目 `main` 中已移除的浏览器填答兜底在 Go 版中也不提供。
